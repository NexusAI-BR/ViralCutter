...
def render_preview_video(font, size, color, highlight, outline, outline_thick, shadow, shadow_sz, bold, italic, upper,
    ...
    def hex_to_ass(h):
        try:
            ...
            print(f"[WARN] Failed to write preview debug input log: {e}")
        ...
        if hex_clean.lower().startswith("rgb"):
            try:
                ...
                print(f"[WARN] Failed to parse RGB preview color '{h}': {e}")
                ...
                for f in os.listdir(preview_dir):
                    if f.startswith("preview_render_") and f.endswith(".mp4") and f != os.path.basename(cache_bust_path):
                        try:
                            ...
                            print(f"[WARN] Failed to remove old preview render '{f}': {e}")
            except OSError as e:
                print(f"[WARN] Failed to list preview directory for cleanup: {e}")
