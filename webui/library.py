...
def get_existing_projects(search_term=None, status_filter=None):
    try:
        projects = db.list_projects(search_term=search_term, status_filter=status_filter)
        return [p.name for p in projects]
    except Exception as e:
        print(f"Error listing projects from DB: {e}")
...
def refresh_projects(search_term=None, status_filter=None):
    projs = get_existing_projects(search_term=search_term, status_filter=status_filter)
    return gr.update(choices=projs, value=None)

def delete_project_action(project_name):
    if not project_name:
        return i18n("No project selected."), refresh_projects()
    
    project = db.get_project_by_name(project_name)
    if not project:
        return i18n("Project not found in database."), refresh_projects()
    
    project_id = project.id
    project_path = project.path
    
    # 1. Delete from DB
    success, path_db = db.delete_project(project_id)
    
    if success:
        # 2. Permanent file deletion (User confirmed)
        try:
            if os.path.exists(project_path):
                shutil.rmtree(project_path)
                msg = i18n("Project and files deleted successfully.")
            else:
                msg = i18n("Project entry removed. Folder was already missing.")
        except Exception as e:
            msg = i18n("DB entry removed, but failed to delete files: {}").format(e)
        
        return msg, refresh_projects()
    else:
        return i18n("Failed to delete project from database."), refresh_projects()
    ...
    try:
        ...
        for i, seg in enumerate(segments_list):
            ...
            try:
                ...
                print(f"[WARN] Invalid score format for segment '{title}': {e}")
