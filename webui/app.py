...
def _append_debug_line(filename, message):
    try:
        with open(filename, "a", encoding="utf-8") as f:
            f.write(message)
    except OSError as e:
        print(f"[WARN] Failed writing debug log '{filename}': {e}")

# Helpers
def convert_color_to_ass(hex_color, alpha="00"):
    _append_debug_line("debug_colors.log", f"INPUT: '{hex_color}'\n")
    ...
    if hex_clean.lower().startswith("rgb"):
        try:
            ...
            if len(nums) >= 3:
                ...
                _append_debug_line("debug_colors.log", f"PARSED RGB: {ret}\n")
                return ret
        except Exception as e:
            _append_debug_line("debug_colors.log", f"RGB ERROR: {e}\n")
    ...
    if len(hex_clean) == 6:
        ...
        _append_debug_line("debug_colors.log", f"PARSED HEX: {ret}\n")
        return ret
        
    _append_debug_line("debug_colors.log", "INVALID: Defaulting to White\n")
    return f"&H{alpha}FFFFFF&"
...
def run_viral_cutter(input_source, project_name, url, video_file, segments, viral, themes, min_duration, max_duration, model, ai_backend, api_key, ai_model_name, chunk_size, workflow, face_model, face_mode, face_detect_interval, no_face_mode, 
    ...
    if use_custom_subs:
        ...
        try:
            ...
            print(f"[WARN] Failed to write temporary subtitle config: {e}")
    
    # --- Database Integration ---
    final_project_name = project_name if input_source == "Existing Project" else (project_name_upload if input_source == "Upload Video" else f"YouTube_{int(time.time())}")
    
    # Check if exists, else create
    project_db = db.get_project_by_name(final_project_name)
    if not project_db:
        # Determine path
        if input_source == "Existing Project":
             current_proj_path = os.path.join(VIRALS_DIR, project_name)
        elif input_source == "Upload Video":
             current_proj_path = project_path
        else:
             # This will be refined during download, let's use a temp for now
             current_proj_path = os.path.join(VIRALS_DIR, final_project_name)
             
        db_id = db.add_project(name=final_project_name, path=current_proj_path, url=url, workflow=workflow_map.get(workflow, "1"))
    else:
        db_id = project_db.id
    
    db.update_project_status(db_id, "completed")
    # ----------------------------
        ...
        if current_process:
            if current_process.stdout:
                try:
                    ...
                    print(f"[WARN] Failed to close process stdout: {e}")
            if current_process.poll() is None:
                ...
                try:
                    ...
                    print(f"[WARN] Failed waiting for process termination: {e}")
            current_process = None
...
if os.path.exists(css_path):
    ...
    css = """
    /* Fallback CSS */
    body, .gradio-container { background-color: #0b0b0b !important; color: #ffffff !important; }
        ...
        with gr.Tab(i18n("Create New")):
             with gr.Row():
                with gr.Column(scale=1):
                    ...
                    with gr.Row():
                        min_dur_input = gr.Number(label=i18n("Min Duration (s)"), value=0, info=i18n("0 = Dynamic (context-driven)"))
            ...
            def render_single(json_path, use_custom, font_name, font_size, font_color, highlight_color, 
                ...
                if use_custom:
                    ...
                    try:
                        ...
                        print(f"[WARN] Failed to write temp subtitle config for single render: {e}")
                else:
                    ...
                    try:
                        ...
                        print(f"[WARN] Failed to remove temp subtitle config: {e}")
            ...
            def render_all(proj_name, use_custom, font_name, font_size, font_color, highlight_color, 
                ...
                if use_custom:
                    ...
                    try:
                        ...
                        print(f"[WARN] Failed to write temp subtitle config for batch render: {e}")
        ...
        with gr.Tab(i18n("Library")):
             with gr.Row():
                with gr.Column(scale=4):
                    lib_search = gr.Textbox(placeholder=i18n("Search project name..."), label=i18n("Search"), show_label=False)
                with gr.Column(scale=2):
                    lib_status_filter = gr.Dropdown(choices=["All", "downloading", "transcribing", "analyzing", "cutting", "finalizing", "completed", "error"], value="All", label=i18n("Status Filter"), show_label=False)
                with gr.Column(scale=1):
                    lib_refresh_btn = gr.Button("🔄", variant="secondary")

             with gr.Row():
                with gr.Column(scale=5):
                    project_dropdown = gr.Dropdown(choices=library.get_existing_projects(), label=i18n("Select Project"), interactive=True)
                with gr.Column(scale=1):
                    lib_delete_btn = gr.Button(i18n("Delete"), variant="stop")

             lib_status_msg = gr.Textbox(label=i18n("Status"), visible=False)
             project_gallery_html = gr.HTML()
             
             # Listeners
             def refresh_lib(search, status):
                 return library.refresh_projects(search, status)

             lib_search.change(refresh_lib, inputs=[lib_search, lib_status_filter], outputs=project_dropdown)
             lib_status_filter.change(refresh_lib, inputs=[lib_search, lib_status_filter], outputs=project_dropdown)
             lib_refresh_btn.click(refresh_lib, inputs=[lib_search, lib_status_filter], outputs=project_dropdown)

             project_dropdown.change(library.generate_project_gallery, inputs=[project_dropdown], outputs=project_gallery_html)
             
             def delete_action(name):
                 if not name: return i18n("No project selected."), gr.update()
                 msg, projs_update = library.delete_project_action(name)
                 return msg, projs_update, gr.update(value=None)

             lib_delete_btn.click(delete_action, inputs=[project_dropdown], outputs=[lib_status_msg, project_dropdown, project_gallery_html])
             lib_status_msg.change(lambda x: gr.update(visible=True if x else False), inputs=[lib_status_msg], outputs=[lib_status_msg])
...
if __name__ == "__main__":
    ...
    if args.colab:
        ...
        try:
            ...
            print(f"[WARN] gr.set_static_paths is not available in this Gradio version: {e}")
