...
def infer_language_from_subtitle_filename(sub_path):
    """
    Infer language code from subtitle filename, e.g.:
    input.pt.srt, input.pt-br.srt, input.en.vtt.
    Returns WhisperX-compatible language code when possible.
    """
    name = os.path.basename(sub_path).lower()
    match = re.search(r"input(?:\.([a-z]{2}(?:-[a-z]{2})?))?\.(?:srt|vtt)$", name)
    if not match:
        return None
    lang = match.group(1)
    if not lang:
...
def apply_safe_globals_hack():
    ...
    try:
        ...
        if hasattr(torch.serialization, 'add_safe_globals'):
            ...
            print("Aplicado patch de seguranca para globals do Omegaconf.")
        ...
        print("Aplicado monkeypatch em torch.load para forcar weights_only=False.")
    ...
    try:
        ...
        for line in lines:
            ...
            if "-->" in line:
                ...
                # E texto (se ja tivermos timestamps)
...
def transcribe(input_file, model_name='large-v3', project_folder='tmp', language=None):
    print(i18n(f"Iniciando transcricao de {input_file}..."))
    ...
    if os.path.exists(srt_file) and os.path.exists(tsv_file) and os.path.exists(json_file):
        print(f"Os arquivos SRT, TSV e JSON ja existem. Pulando a transcricao.")
    ...
    try:
        ...
        # 1. Carregar Audio (sempre necessario)
        print(f"Carregando audio: {input_file}")
        audio = whisperx.load_audio(input_file)
        
        # 2. Verificar se existem legendas baixadas para Alignment Only
        # Procurar por input.<lang>.srt/vtt e, por ultimo, input.srt/vtt.
        potential_subs = []
        potential_subs.extend(sorted(glob.glob(os.path.join(output_folder, "input*.srt"))))
        potential_subs.extend(sorted(glob.glob(os.path.join(output_folder, "input*.vtt"))))
        
        start_segments = None
        alignment_only = False
        
        # Default: usar idioma fornecido ou None (auto-detect)
        ...
        if potential_subs:
            ...
            if parsed and len(parsed) > 0:
                inferred_lang = infer_language_from_subtitle_filename(sub_path)
                if inferred_lang:
                    # Se o usuario forcou um idioma e a legenda nao bate, ignorar
                    if language and inferred_lang != language:
                        print(f"Legenda externa em '{inferred_lang}' ignorada (idioma forcado: '{language}').")
                    else:
                        start_segments = parsed
                        alignment_only = True
                        detected_language = inferred_lang
                        print(f"Idioma inferido da legenda para alinhamento: {detected_language}")
                        print("--- MODO ALINHAMENTO RAPIDO ATIVADO ---")
                else:
                    print("Legenda externa sem idioma no nome. Ignorando alignment-only para evitar idioma incorreto.")

        result = None
        
        if alignment_only and start_segments:
            # Pular Transcricao, ir direto para Alinhamento
            print("--- MODO ALINHAMENTO RAPIDO ATIVADO ---")
            pass 
        else:
            # 3. Transcrever (Caminho Normal)
            print("Nenhuma legenda valida encontrada. Realizando transcricao completa (WhisperX)...")
            ...
            # Se language foi fornecido, forcar o idioma na transcricao
            if language:
                transcribe_kwargs['language'] = language
                print(f"Idioma forcado para transcricao: {language}")
        ...
        print(f"Alinhando transcricao (Idioma: {detected_language}) para obter timestamps precisos...")
        ...
        try:
            ...
            # aligned_result agora contem "segments" com word timestamps
            ...
            if alignment_only:
                 print("Falha critica no alinhamento de legendas externas. Abortando usage de legendas externas.")
                 # Opcional: Fallback para transcricao normal se falhar? Seria complexo aqui pois ja limpamos memoria.
                 # Vamos apenas salvar o que temos (timestamps da legenda original podem nao bater com audio perfeitamente se nao alinhar)
                 result = {"segments": start_segments, "language": detected_language}
            else:
                 print("Continuando com transcricao bruta.")
        ...
        # Se veio do alignment_only, result e {'segments': [...], ...}
        # Se o alinhamento falhou, result tem segments originais.
        
        # WhisperX writers esperam um dicionario result com chaves 'segments', 'language'.
        ...
        print(f"Processamento concluido em {int(elapsed//60)}m {int(elapsed%60)}s.")

    except Exception as e:
        print(f"ERRO CRITICO na transcricao: {e}")
        import traceback
        traceback.print_exc()
        raise

    if not os.path.exists(srt_file):
        print(f"AVISO: Arquivo SRT {srt_file} nao encontrado apos execucao.")
