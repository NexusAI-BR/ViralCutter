...
def call_gemini(prompt, api_key, model_name='gemini-2.5-flash-lite'):
    if not HAS_GEMINI:
...
def _expand_segment_for_context(transcript_segments, start_idx, end_idx, min_duration, max_duration):
    ...
    - adds a little lead-in context,
    - ensures a minimum narrative duration,
    - tries to finish on a natural sentence boundary.
    """
    if not transcript_segments:
    ...
    while start_idx > 0:
        ...
        prev_start = transcript_segments[prev_idx]['start']
        if start_time - prev_start > lead_in_cap:
            break
        candidate_duration = end_time - prev_start
        if candidate_duration > max_duration:
            break
        start_idx = prev_idx
        start_time = prev_start
        if _is_sentence_end(transcript_segments[start_idx]['text']):
            break

    # 2) Extend duration based on mode.
    if dynamic_mode:
        # Dynamic mode: no fixed minimum duration, only context-oriented expansion.
        while end_idx < total - 1 and (end_time - start_time) < max_duration:
            # Stop at sentence end once we already have enough context.
            if _is_sentence_end(transcript_segments[end_idx]['text']) and (end_time - start_time) >= soft_target:
                break
            next_idx = end_idx + 1
            next_end = transcript_segments[next_idx]['end']
            if next_end - start_time > max_duration:
                break
            end_idx = next_idx
            end_time = next_end
    else:
        # Fixed-min mode: expand until preferred minimum duration.
        while end_idx < total - 1 and (end_time - start_time) < preferred_min:
            next_idx = end_idx + 1
            next_end = transcript_segments[next_idx]['end']
            if next_end - start_time > max_duration:
                break
    ...
    # 3) If possible, finish at a sentence boundary.
    while end_idx < total - 1 and (end_time - start_time) < max_duration:
        min_for_boundary_stop = hard_min if hard_min > 0 else 0
        if _is_sentence_end(transcript_segments[end_idx]['text']) and (end_time - start_time) >= min_for_boundary_stop:
            break
        next_idx = end_idx + 1
        next_end = transcript_segments[next_idx]['end']
        if next_end - start_time > max_duration:
...
def process_segments(raw_segments, transcript_segments, min_duration, max_duration, output_count=None):
    ...
    tempo_minimo = max(0, min_duration or 0)
    ...
    for seg in all_segments:
        try:
            ...
            if match_start_idx == -1:
                match_start_idx = start_idx
            if match_end_idx == -1:
                match_end_idx = match_start_idx

            expanded_start_idx, expanded_end_idx = _expand_segment_for_context(
                transcript_segments,
                match_start_idx,
                match_end_idx,
                tempo_minimo,
            ...
            final_start_time = transcript_segments[expanded_start_idx]['start']
            final_end_time = transcript_segments[expanded_end_idx]['end']
            
            # Calculate Duration
            duration = final_end_time - final_start_time
            
            # Validate Duration (Min)
            if tempo_minimo > 0 and duration < tempo_minimo: 
    ...
    # Validacao basica de que temos start_time
    validated_segments = []
...
def create(num_segments, viral_mode, themes, tempo_minimo, tempo_maximo, ai_mode="manual", api_key=None, project_folder="tmp", chunk_size_arg=None, model_name_arg=None):
    # Pedir mais segmentos quando usando IA com revisor, para ter margem apos filtragem
    if ai_mode in ("gemini", "g4f"):
        quantidade_de_virals = num_segments * 2
        print(f"[INFO] Pedindo {quantidade_de_virals} segmentos (2x) para ter margem apos revisao de qualidade.")
    else:
    ...
    if ai_mode == "gemini":
        ...
        cfg_model = config["gemini"].get("model", "gemini-2.5-flash-lite")
    ...
    if os.path.exists(prompt_path):
        ...
        print("Aviso: prompt.txt nao encontrado. Usando prompt interno.")
...
{json_template}"""
    ...
    # Dynamic duration mode by default when min_duration <= 0:
    # let the model choose context-complete duration up to max_duration.
    if tempo_minimo is None or tempo_minimo <= 0:
        system_prompt_template = system_prompt_template.replace(
            ...
            "CONSTRAINT: No fixed minimum duration. Choose the time needed for full context (start, middle, end), up to {max_duration}s."
        )
        system_prompt_template = system_prompt_template.replace(
            "Constraints: Each segment MUST be between {min_duration} seconds and {max_duration} seconds.",
            "Constraints: No fixed minimum duration. Choose the time needed for full context (start, middle, end), up to {max_duration} seconds."
        )
    ...
    for i, prompt in enumerate(output_texts):
        ...
        if ai_mode == "manual":
            ...
            print("Dica: Se o JSON tiver multiplas linhas, tente colar tudo de uma vez ou minificado.")
            ...
            if user_input.lower() == 'file':
                try:
                    ...
                    print(f"Arquivo {response_json_path} nao encontrado.")
            else:
                response_text = user_input
                if response_text.strip().startswith("{") and not response_text.strip().endswith("}"):
                    print("Parece incompleto. Cole o resto e de Enter (ou Ctrl+C para cancelar):")
                    try:
                        rest = sys.stdin.read() 
                        response_text += rest
                    except:
