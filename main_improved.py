...
def get_subtitle_config(config_path=None):
    ...
    config = {
        ...
        "vertical_position": 120, # Ajustado para enquadrar melhor em vídeo vertical 9:16
...
def main():
    ...
    parser.add_argument("--min-duration", type=int, default=0, help="Minimum segment duration (seconds). Use 0 for dynamic/context-driven cuts")
    parser.add_argument("--max-duration", type=int, default=90, help="Maximum segment duration (seconds)")
    parser.add_argument("--language", default=None, help="Force transcription language (e.g. 'pt' for Portuguese, 'en' for English). If not set, auto-detect.")
    ...
    parser.add_argument("--skip-review", action="store_true", help="Skip AI quality review of generated segments")
    ...
    # Lazy imports: keep CLI responsive (e.g. --help) and allow partial workflows
    # even when optional heavy dependencies are missing.
    try:
        from scripts import (
            download_video,
        ...
        print(i18n("Error importing core pipeline modules: {}").format(e))
        sys.exit(1)

    transcribe_video = None
    if args.workflow != "3":
        try:
            from scripts import transcribe_video
        except Exception as e:
            print(i18n("Error importing transcription module: {}").format(e))
            print(i18n("Tip: install missing dependencies with install_dependencies.bat"))
            sys.exit(1)
    ...
    try:
        ...
        if workflow_choice != "3":
            ...
            if not viral_segments:
                ...
                if not args.skip_review and ai_backend in ("gemini", "g4f"):
                    try:
                        from scripts.review_segments import review_and_refine
                        review_transcript = create_viral_segments.load_transcript(project_folder)
                        viral_segments = review_and_refine(
                            viral_segments,
                            review_transcript,
                            ai_mode=ai_backend,
                            api_key=api_key,
                            model_name=model_name if 'model_name' in dir() else (api_config.get(ai_backend, {}).get('model', '')),
                            min_duration=args.min_duration,
                            max_duration=args.max_duration,
                            min_score=70
                        )
                        if not viral_segments or not viral_segments.get("segments"):
                            print(i18n("Error: All segments were rejected by the quality reviewer."))
                            print(i18n("Try with --skip-review or adjust your video content."))
                            sys.exit(1)
                    except Exception as e:
                        print(f"[WARN] Revisão de qualidade falhou: {e}. Continuando com segmentos originais.")
                else:
                    if args.skip_review:
                        print(i18n("Quality review skipped (--skip-review flag)."))
                # ---------------------------------
